...
is_deeply App::Sqitch::Command::config->configure( $sqitch->config,
    { local => 1, } ), { context => 'local', }, 'Local context should be local';
...
is_deeply \@emit, [ [1] ], 'Should have emitted the revert revision as an int';
@emit = ();
...
ok $cmd->execute('bundle.tags_only'), 'Get bundle.tags_only as bool';
is_deeply \@emit,
  [ [ 'bundle.tags_only=' . ( $Config::GitLike::VERSION > 1.08 ? 'true' : 1 ) ]
  ],
  'Should have emitted bundle.tags_only as a bool';
@emit = ();
