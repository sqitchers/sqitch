...
=head1 Synopsis
  ...
  sqitch [options] add --change --name sprockets pg sql
...
use of the core C<--engine>, C<--top-dir>, C<--plan-file>, C<--extension>,
C<--deploy-dir>, C<--revert-dir>, and C<--verify-dir> directories. You
...
By default, the C<add> command will add the change to all plans in the Sqitch
configuration. It will iterate over all known targets, add it to all the
plans, and write the files required for each target. This works well for
projects in which there is a single plan with separate top directories for
each engine, for example.

To limit the plans and top directories to which changes and their scripts will
be added, specify the target, engine, or plan file names to add to. Simply
pass them as arguments after the change name. Use C<--change> to disambiguate
the change name from the other parameters if necessary (or preferable). See
...
Add a change only to the plan used by the C<vertica> engine in a project:
  ...
  sqitch add --name logs vertica -n 'Adds the logs table to Vertica.'
