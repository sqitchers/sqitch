...
use of the core C<--top-dir>, and C<--plan-file> options. You probably don't
want to often use any of those, but should define alternatives on a per-engine
or per-target basis. See L<sqitch-configuration> for details.
...
Path to directory containing the plan file. Overrides target, engine, and core
configuration values. Defaults to C<.>.
