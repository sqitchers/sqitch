...
Special symbolic name for the first change deployed to the database from the
current project.

=item C<@LAST>

Special symbolic name for the last change deployed to the database from the
current project.

=item C<< <change>^ >>, e.g., C<@HEAD^^>, C<@LAST^3>, C<@beta^2>
...
=item C<< <change>~ >>, e.g., C<@ROOT~>, C<@FIRST~~>, C<@bar~4>
