...
Relatedly, one engine-specific L<sqitch> option, C<--client> will also be used
to write the engine-specific variable C<core.$engine.client>. Again, if this
option is not specified, the engine-specific option will still be written out,
but as a comment with the user or system configuration value, or a default
value.
