...
F<sqitch.conf>. While the C<init> command has no options of its own, it
relies on several of the options to C<sqitch> to determine what to write to
this file:
...
Relatedly, the engine-specific L<sqitch> options will also be used to write
