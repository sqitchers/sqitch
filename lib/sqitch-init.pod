...
Sets the C<core.$engine.client> configuration variable if C<--engine> is also
passed.

=item C<--registry>

Sets the C<core.$engine.registry> configuration variable if C<--engine> is also
passed.

=back

As a general rule, you likely won't need any of them except for C<--engine>,
since many commands need to know what engine to use, and specifying it on the
