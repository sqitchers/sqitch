...
=head1 Synopsis
  ...
  sqitch target add [-u <uri>] [-r <registry>] [-c <client>] <name> <uri>
  sqitch target update [-u <uri>] [-r <registry>] [-c <client>] <name> <uri>
...
Set the URI for the target. Used only by the C<add> and C<update> actions.
...
Set the registry for the target. Used only by the C<add> and C<update>
actions.
...
Set the client for the target. Used only by the C<add> and C<update> actions.
...
Update the URI, client, and/or registry configuration for target C<< <name> >>.
...
Gives some information about the remote C<< <name> >>.
