...
change note, for example). If unset, C<$VISUAL> or C<$EDITOR> will be used. If
C<$VISUAL> and C<$EDITOR> are not set, the C<core.editor> configuration variable
will be consulted. If it's not set, Sqitch will invoke C<notepad.exe> on Windows
and C<vi> elsewhere.
