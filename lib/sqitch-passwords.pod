...
Some database engines allow a password to be specified via an environment
variable. However use of these environment variables is not recommended for
security reasons, as some operating systems allow non-root users to see
process environment variables via C<ps>.

Given that caveat, for those that do support a password environment variable,
Sqitch encapsulates this functionality in an environment variable of its own,
C<$SQITCH_PASSWORD>. You may use this one environment variable in place of any
of the engine-specified environment variables listed below:
