...
Note that I've used the C<--to> option to revert only to the C<appuser> step.
...
Here I've taken advantage of the C<--requires> option to have Sqitch write the
deploy file with the C<-- requires:> line already filled in. Have a look:
...
write the code now. Here's what C<sql/deploy/insert_user.sql> should look
like:
...
Note the use of C<--to ^^> to revert two steps. We also could have used C<--to
fa650af> to revert to the tag for the C<users> step, or just used C<--to
users> to revert to that step. Let's do the commit and re-deploy dance:
...
release via the tag. So let's bundle it up.
  ...
  Bundling in bundle
    - appuser at v1.0.0-dev1
    - users at v1.0.0-dev1
    - change_pass at v1.0.0-dev1
    - insert_user at v1.0.0-dev1
...
it as a simple list. Notice that the SHA1's for each step are not present; the
