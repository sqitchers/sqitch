...
If user "foo" ever got access to the database, she could immediately see that
user "bar" has the same password and thus access the account. Not a great
idea. So we need to modify the C<insert_user()> and C<change_pass()> functions
to fix that. How? we'll use
...
Now, normally, doing this sort of thing in database changes is a PITA. You have
...
Create a new step to add the
L<C<pgcrypto>|http://www.postgresql.org/docs/current/static/pgcrypto.html>
extension.
...
Edit the function in F<sql/deploy/insert_user_crypt.sql> to switch from
C<MD5()> to C<crypt()> and to add a dependency on the C<pgcrypto> step.
...
Test the changes to make sure you can deploy and revert C<insert_user_crypt>.
...
That doesn't seem I<too> bad. And you can do it with Sqitch, too, if you
prefer. But it makes for rather bad VCS management. This is effectively a
one-line change to the functions, but we're copying their entire contents to
new files. Hell, the reversion script is I<identical> to the original deploy
script.
...
That makes much more sense. Wouldn't it be great if we could just make I<that>
change, commit it to the VCS, and have the deployment and reversion "just
work"?