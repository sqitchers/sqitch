...
who will have only limited access to objects in the database. Run this command:
  ...
  > sqitch add-change appuser
  Created sql/deploy/appuser.sql
  Created sql/revert/appuser.sql
  Created sql/test/appuser.sql

The C<add-change> command creates deploy and revert scripts that represent a new
change change. Now we edit these files. The C<deploy> script's job is to create
...
users, of course, so we'll create a table for them. First, add the new change:
  ...
  > sqitch add-change users --requires appuser
...
the C<appuser> change. Although that change has already been added and committed,
and therefore should always be applied before the C<users> change, it's a good
idea to be explicit about dependencies. This becomes especially important when
committing a number of changes at one time, such as when merging a patch or a
branch.
...
colon and then a comma-delimited list of required changes. The requirements may
...
any part of the deployment fails, the whole change fails. Such may work
...
which represents the deployment of the C<appuser> change. (You can see this tag
...
Let's add a couple more changes to add functions for managing users.
  ...
  > sqitch add-change insert_user --requires users --requires appuser
  Adding sql/deploy/insert_user.sql
         -- requires: users, appuser
  Adding sql/revert/insert_user.sql

  > sqitch add-change change_pass --requires users --requires appuser
...
Now we can add a new change to create a table for our flips.
  ...
  > sqitch add-change flips --requires users
