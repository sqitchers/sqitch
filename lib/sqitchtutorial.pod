...
uses a schema to namespace its metadata; MySQL and SQLite use a prefix). But
if you connect to the database, you'll be able to see the role:
...
Note the empty string for the tag. That's there because the step is untracked
-- that is, not yet committed to our repository or added to a plan file. (More
on plan files later.) The C<status> command will show similar informtion:

  > sqitch -d flipr_test status
  # Deployed to "".
  # Last step deployed 2012-04-09 18:43:45: "appuser".
  #
...
Great, no records. The status message should reflect as much:
...
We still have a record that it happened, though, in the C<sqitch.history>
table:
...
auditing purposes. The C<log> command shows similar information:
...
Note that it has properly recorded the dependency on the C<appuser> state. But
the tag is empty because the deployment has not yet been committed to Git. In
general, we want tags, so let's revert the change:
