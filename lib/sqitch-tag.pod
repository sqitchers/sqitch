...
multi-plan projects in which tags should be kept in sync. Overrides the value
of the C<tag.all> configuration; use C<--no-all> to override a true C<tag.all>
configuration.
...
List the tags or add the new tag to all the plans in a project. Useful for
multi-plan projects in which tags should be kept in sync.
