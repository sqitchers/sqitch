...
sqitch-status - Show the current database deployment status
...
Show timestamps in the specified format length, using the system locale.
...
Format to use for timestamps. Supports same values as the C<--date-format>
option.
