...
By default, all project plans known to the Sqitch configuration will be
bundled, along with their scripts. This is useful for packaging up a project
for multiple database engines. If, however, you'd like to limit a bundle to
just a subset of plans, identify the plans on the command-line by passing one
...
after each target agument. In either case, the changes can be specified in any
way documented on L<sqitchchanges>. See L</Examples> for examples.
