...
=item * C<mysql> - L<MySQL|http://dev.mysql.com/>
...
=item C<core.pg.target>

PostgreSQL database target, either the name of target managed by the
L<C<target>|sqitch-target> command, or a database connection URI.

=item C<core.pg.uri>

PostgreSQL database connection URI. Usually looked up from the associated
target, but a default may be specified here.

=item C<core.pg.registry>

The name of the Sqitch registry schema. Sqitch will store its own data in this
schema. No other data should be stored there. Defaults to C<sqitch>.
...
=item C<core.sqlite.target>

SQLite database target, either the name of target managed by the
L<C<target>|sqitch-target> command, or a database connection URI.

=item C<core.sqlite.uri>

SQLite database connection URI. Usually looked up from the associated target,
but a default may be specified here.

=item C<core.sqlite.registry>

Name to use for the Sqitch registry database. Sqitch will store its own data
in this database. If the value look like an absolute path, then it will be the
database file. Otherwise, it will be in the same directory as the database.
Defaults to C<sqitch>.

=back

=head3 C<core.mysql>

Configuration properties for the MySQL engine.

=over

=item C<core.mysql.client>

Path to the C<mysql> command-line client. Defaults to the first instance
found in the path.

=item C<core.mysql.target>

MySQL database target, either the name of target managed by the
L<C<target>|sqitch-target> command, or a database connection URI.

=item C<core.mysql.uri>

MySQL database connection URI. Usually looked up from the associated target,
but a default may be specified here.

=item C<core.mysql.registry>

The name of the Sqitch registry database. Sqitch will store its own data in
this database. Defaults to C<sqitch>. If you need to manage multiple databases
on a single server, and don't want them all to share the same registry, change
this property to a value specific for your database.
...
=item C<core.oracle.target>

Oracle database target, either the name of target managed by the
L<C<target>|sqitch-target> command, or a database connection URI.

=item C<core.oracle.uri>

Oracle database connection URI. Usually looked up from the associated target,
but a default may be specified here.

=item C<core.oracle.registry>

The name of the Sqitch registry schema. Sqitch will store its own data in this
schema. No other data should be stored there. Uses the current schema by
