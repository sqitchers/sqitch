...
The command to use as a pager program. This overrides the C<PAGER> environment
variable on UNIX like systems. In case both C<core.pager> and C<PAGER> are
not set, a best-effort search among the commonly installed pager programs like
C<less>, C<more> is performed.
