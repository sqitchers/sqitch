...
The hypothetical C<set> key in the C<core.revert> section might need to set C<foo>
to "hi" instead of "bar". Here's how to make that change:

  % sqitch config core.revert.set foo=bar foo=hi

To delete the entry for C<bundle.from>, do

  % sqitch config --unset bundle.from

If you want to delete an entry for a multivalue setting (like C<core.revert.set>),
...
Or, to query a multivalue setting for only those values that match C</ba/>:
  ...
  % sqitch config --get core.revert.set ba

If you want to know all the values for a multivalue setting, do:

  % sqitch config --get-all core.revert.set

If you like to live dangerously, you can replace all C<core.revert.set> with a
new one with

  % sqitch config --replace-all core.revert.set bring=funk

However, if you only want to replace the line without C<baz>, do something
like this:

  % sqitch config core.revert.set = '!baz'
...
=item * C<vertica> - L<Vertica|http://https://my.vertica.com/>
...
the key C<core.$engine> where C<$engine> may be any value accepted for
C<core.engine>.

=over

=item C<core.$engine.target>
