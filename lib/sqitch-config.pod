...
=item C<core.sqlite.sqitch_database>

Sqitch metadata SQLite database connection URI.
...
=item C<core.mysql.sqitch_db>

The name of the database (schema) in which Sqitch metadata will be stored. No
other data should be stored in this database. Defaults to C<sqitch>. If you
need to manage multiple databases on a single server, and don't want them all
to share the same Sqitch metadata database, change this property to a value
