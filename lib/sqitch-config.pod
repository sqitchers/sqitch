...
that do not match the regex, just prepend a single C<!> (exclamation maark) in
...
form (simple integer for int, decimal number for num, a "true" or "false"
string for bool) If no type specifier is passed, no checks or transformations
...
"foo" to "hi" instead of "bar". Here's how to make that chnage:
...
Or, to query a multivalue settting for only those values that match C</ba/>:
...
However, if you only want to replace the line without "baz", do something like
this:
