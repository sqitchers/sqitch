...
This document explains how this evaulation works, and how to use the
L<C<init>|sqitch-init>, L<C<config>|sqitch-config>,
L<C<engine>|sqitch-engine>, and L<C<target>|sqitch-target> commands to
configure these values for various deployment scenearios.
...
The C<[core]> section contins default configurations, the most important of
...
specify a target URI when deploying to their own databasses. The C<registry>
...
configuration, use the L<C<engine>|sqitch-engine> command's C<add> acction:
...
Sqitch will fall back on the default engine, defind by the C<core.engine>
...
Then copy the templated scripts to the F<sqlite> directory:
...
commands. For example, to deploy to a PostgresSQL database named "widgets" on
...
add a target for our SQLite database, too. Maybe it's used for dev?
...
Now, any management of the C<prod-billing> target target will use the
F<target.plan> plan file.
