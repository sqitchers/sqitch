...
sqitch-add-change - Add a deployment change

=head1 Synopsis

  sqitch [options] add-change [<template-options>] name
...
Location to look for the templates. If none is specified, C<add-change> will
...
can be added via the C<--set> option, like so:
  ...
  sqitch add-change --set transations=1 --set schema=foo
...
C<[add-change "variables]> L<config|sqitch-config> section (see 
...
=item C<add-change.template_directory>
...
=item C<add-change.with_deploy>

=item C<add-change.with_revert>

=item C<add-change.with_test>
...
but wish to create them for a given call to C<add-change>, use the appropriate
option: C<--deploy>, C<--revert>, or C<--test>.

=item C<add-change.deploy_template>

=item C<add-change.revert_template>

=item C<add-change.test_template>
